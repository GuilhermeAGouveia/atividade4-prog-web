...
const htmlElements = () => ({
  ...
  buttonsPagination: {
    ...
    numericButtonsContainer: document.querySelector(
      "#pagination-buttons > .container-button-pag"
    ),
  ...
  `
    <tr id="tableLine${element.id}">
      ...
      <div class="status-container">
          <input type="checkbox" class="status" id=${
            element.id
          } onclick="handleUpdateStatus(${element.id}, this.checked)" ${
...
//Insere cada linha na tabela, cada elemento do array `customers` vira uma linha
...
function blockButtonsPagination(direction, block) {
  let { leftButton: left, rightButton: right } =
    htmlElements().buttonsPagination;
...
function defineButtonsPagination(nPages, pageSelect) {
  let { numericButtonsContainer: containerPagination } =
    htmlElements().buttonsPagination;
...
async function handleUpdateStatus(id, status) {
  ...
  showBanner('error', 'Funcionalidade indisponível')
...
class Pagination {
  ...
  requestPage = async () => {
    let stringRequest = this.searchString
      ? `search?q=${this.searchString}&page=${this.pageNumber}&size=${this.pageSize}`
      : `?page=${this.pageNumber}&size=${this.pageSize}`;
    console.log(stringRequest);
    let { data: customers } = await api.get(`customers/`);
    this.elements = customers
    return {customers, totalSize: customers.length};
  };

  async showPage() {
    let { customers, totalSize } = await this.requestPage();
...
class SortPage {
  ...
  addFilterButtonsInFields() {
    let fields = ["id", "phoneNumber", "firstName", "lastName", "emailAddress"];
    const { headers: tableHeaders } = htmlElements().table;
    let table = document.querySelectorAll("#customers > thead > tr > th");
    for (let i = 0; i < fields.length; i++) {
      tableHeaders[i]
        .querySelector(".contentHeader")
        .appendChild(
          this.createButtonFilter("sort-button-customers" + i, fields[i])
        );
    }
  }
}
